...
export async function getLatestCommitSha(): Promise<string> {
  ...
  try {
    ...
    gitHubStatus.value = `Error getting latest commit: ${error.message}`
    throw error
  }
}
...
export async function getFileContent(path: string): Promise<string> {
  ...
  try {
    ...
    gitHubStatus.value = `Error getting file content: ${error.message}`
    throw error
  }
}
...
export async function updateReference(commitSha: string): Promise<void> {
  ...
  try {
    ...
    gitHubStatus.value = `Error updating reference: ${error.message}`
    throw error
  }
}
...
export async function commitChanges(
  ...
  try {
    ...
    const fsNodes = rootNode.kind === 'directory' && rootNode.children
      ? rootNode.children
      : [rootNode]
...
export async function getChangedFiles(): Promise<string[]> {
  ...
  try {
    ...
    const changedFiles = data.files.map((file: any) => file.filename)

    gitHubStatus.value = `Found ${changedFiles.length} changed files`
    return changedFiles
  } catch (error) {
    gitHubStatus.value = `Error checking for changes: ${error.message}`
    throw error
  } finally {
    isGitHubBusy.value = false
  }
